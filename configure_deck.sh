...
create_dialog() {
    while true; do
        readarray -t chosen_tasks < <(echo $menu | xargs zenity --height=800 --width=1280 --list --checklist --column="command" --column="task" --column="description" --column="notes" --hide-column=2 --print-column=2 --separator=$'\n')
        run_tasks
    done
}
...
set_menu() {
    ...
    FALSE "install_flatpaks" "Install Flatpaks" "Install Flatpaks from a list"
    FALSE "save_flatpaks_install" "Save Flatpaks List" "Save list of Flatpaks to easily install later"
    FALSE "install_proton_ge_in_steam" "Install Proton GE in Steam" "The ProtonGE tar needs to be in the steam-deck-configurator folder"
    FALSE "install_bauh" "Install Bauh" "The Bauh AppImage needs to be in the steam-deck-configurator folder"
